...
class GNN_Model(tf.keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        for it in range(int(self.config['HYPERPARAMETERS']['t'])):
            ...
            path_gru_rnn = tf.keras.layers.RNN(self.path_update,
                                               return_sequences=True,
                                               return_state=True)

            path_state_sequence, path_state = path_gru_rnn(
                inputs=self.masking(tf.concat([queue_input, link_input], axis=2)),
            ...
            path_gather = tf.gather_nd(path_state_sequence, ids_q)
            path_sum = tf.math.unsorted_segment_sum(path_gather, tf.squeeze(inputs['sequence_queues']), n_queues)
            path_sum = tf.ensure_shape(path_sum,
                                       [None, int(self.config['HYPERPARAMETERS']['path_state_dim'])])
